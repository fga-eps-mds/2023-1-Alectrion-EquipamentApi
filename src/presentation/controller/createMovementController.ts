...
import {
  ...
  NullFieldsError,
  InvalidStatusError
} from '../../useCases/createMovement/createMovementUseCase'
...
export class CreateMovementController extends Controller {
  ...
  async perform(
    httpRequest: CreateMovementUseCaseData
    ...
    if (
      response.error instanceof NullFieldsError ||
      response.error instanceof InvalidTypeError ||
      response.error instanceof InvalidEquipmentError ||
      response.error instanceof InvalidStatusError
    )
      return badRequest(response.error)
    if (response.error instanceof InvalidDestinationError)
